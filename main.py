...
""" 
# IS the mac address randomised? 
randomized_count = 0
non_randomized_count = 0
...
for pkt in probe_packets:
    ...
    if is_randomized_mac(
        ...
        non_randomized_count += 1
       
print()
print("=" * 50)
print("MAC RANDOMIZATION STATISTICS")
print("=" * 50)

print(
    f"Randomized packets: {randomized_count}"
)

print(
    f"Non-randomized packets: {non_randomized_count}"
)

total = (
    randomized_count
    +
    non_randomized_count
)

if total > 0:

    print(
        f"Randomized percentage: "
        f"{100 * randomized_count / total:.2f}%"
    )

    print(
        f"Non-randomized percentage: "
        f"{100 * non_randomized_count / total:.2f}%"
    )

# GROUP BY ROUTERS 
router_packets = group_by_router(cleaned_dataset)

eps_values = np.arrange( 
    0.5,
...
for eps in eps_values:
    ...
    router_train_occupancies = {} 
    router_test_occupancies = {}
    router_train_df_features = {}
    router_test_df_features = {}
...
# ROUTER LOOP
    ...
    #SPLIT
        train_packets, test_packets = temporal_split(   # Inside of each Router's packets we perform the split to the train and test packets
...
# ----------------------------------
# SHOW CLUSTERING FOR FIRST 5 WINDOWS
# ----------------------------------
            if eps == 0.5: # Choose 1 eps value just for an example 
            ...
            train_windows[:5]
                ...
                print(
                    f"Probe packets:{len(probe_packets)}"
                )
                ...
                mac_features = build_mac_features(
                probe_packets
                )
        ...
        # Computing the occupancy list of each window for train packets 
        train_occupancies = []
        train_device_free_features = []
        ...
        for window in train_windows:
            ...
            beacon_features = extract_beacon_features(
                beacon_packets
            )

            train_device_free_features.append(
                beacon_features
            )

        
        # After all the windows of the routers are processed 

        router_train_occupancies[router] = ( 
            train_occupancies
        )
        router_train_df_features[router] = (
            train_device_free_features
        )


    # Computing the occupancy list of each window for TEST packets
    # And the device free features per window in each router for TEST packets 

        test_occupancies = [] # list of occupancies per window; no need for separating the data into training and testing parts. 
        test_device_free_features = [] # list of beacon features per window 
        ...
        for window in test_windows: 
            ...
            test_device_free_features.append(
                beacon_features
            )
            

    # After all the windows of the routers are processed 
        router_test_occupancies[router] = (
            test_occupancies
        )

        router_test_df_features[router] = (
            test_device_free_features
        )

    # ----------------------------------
    # THE ML PART - DEVICE_FREE REGRESSION  
    # ----------------------------------

    print()
    print("=" * 80)
    print("DEVICE-FREE REGRESSION")
    print("=" * 80)

    for router in router_train_df_features:
    ...
    #testing 
        predictions, mae = evaluate_model(
    ...
    ) # predicts all windows in a single call

        router_eps_results[
        router
        ][eps] = mae  # dict for each router's mae result for diff eps value 
...
print()
print("=" * 80)
print("BEST EPS PER ROUTER")
print("=" * 80)

for result_router in router_eps_results: # for each router we will estimate the best eps with min mae 

    print()
    print(f"ROUTER: {result_router}")
    print("-" * 40)

    for eps, mae in sorted(
        router_eps_results[result_router].items()
    ):
    ...
    best_eps = min(
        ...
        router_eps_results[result_router], # -> gives us a dict with keys as eps and values as mae 
                                    
        key=router_eps_results[result_router].get # -> as a key for estimating the min we set the mae values (dict.get()) 

    )

    best_mae = (
        router_eps_results[result_router][best_eps]
    )
...
# Print first 5 windows 
        for i in range(5):
            ...
            print(
            f"Device-based: {y_test[i]}"
        )

            print(
            f"Device-free: {predictions[i]:.2f}"
        )

            print(
            f"Difference: "
            f"{abs(predictions[i] - y_test[i]):.2f}"
        ) """